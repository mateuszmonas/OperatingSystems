...
void handle_exit(){
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (client_keys[i] != FREE_SLOT) {
            msg.client_id = i;
            msg.client_key = client_keys[i];
            msgsnd(msgget(client_keys[i], 0), &msg, MSG_SIZE, 0);
        }
    }
...
void echo(chat_msg * msg){
    msgsnd(msgget(client_keys[msg->client_id], 0), msg, MSG_SIZE, 0);
    printf("%s\n", msg->text);
}
...
void connect(chat_msg * msg){
    ...
    if (id1 < MAX_CLIENTS && available[id1] && available[id2]) {
        ...
        msg->client_id = id2;
        msg->client_key = client_keys[id2];
        msgsnd(msgget(client_keys[id1], 0), msg, MSG_SIZE, 0);
        msg->client_id = id1;
        msg->client_key = client_keys[id1];
        msgsnd(msgget(client_keys[id2], 0), msg, MSG_SIZE, 0);
        available[id1] = false;
        available[id2] = false;
        printf("connected\n");
    }
...
void stop(chat_msg * msg){
    client_keys[msg->client_id] = FREE_SLOT;
    client_count--;
    printf("%d left\n", msg->client_id);
}
